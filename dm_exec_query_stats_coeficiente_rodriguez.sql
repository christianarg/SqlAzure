...
, case
	when DATEDIFF(minute, qs.creation_time, qs.last_execution_time) > 0 
		then
			 (qs.execution_count / DATEDIFF(minute, qs.creation_time, qs.last_execution_time)) * qs.total_worker_time
		else
			-1
end as coeficiente_rodriguez
...
ORDER BY coeficiente_rodriguez DESC, qs.last_execution_time DESC